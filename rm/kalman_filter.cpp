...
ArmorKalmanFilter::ArmorKalmanFilter(float dt_) : dt(dt_), has_last_pos(false),
...
// ===================== 延迟补偿预测 =====================
Point2f ArmorKalmanFilter::predictWithDelayCompensation(float delay_time) {
    // 根据总延迟时间进行多步预测
...
// ===================== 带延迟的姿态预测 =====================
Point3f ArmorKalmanFilter::predictPoseWithDelay(float delay_time, float current_pitch, float current_yaw) {
    // 预测延迟后的pitch和yaw
    int prediction_steps = static_cast<int>(delay_time / dt);
...
Point2f ArmorKalmanFilter::predict() {
    state = kf.predict();
...
Point2f ArmorKalmanFilter::update(const Point2f& detect_center) {
...
void ArmorKalmanFilter::init(const Point2f& init_center) {
...
void ArmorKalmanFilter::initWithVelocity(const Point2f& init_center, const Point2f& init_velocity) {
    last_valid_pos = init_center;
    has_last_pos = true;
...
// ===================== 小陀螺检测 =====================
bool ArmorKalmanFilter::detectSpinning(const Point2f& current_center) {
    // 优化：预先检查容量，避免频繁重新分配
...
// ===================== 自适应预测 =====================
Point2f ArmorKalmanFilter::adaptivePredict(const Point2f& current_center) {
